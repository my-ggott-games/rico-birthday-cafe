    ...
    Player: new (
        ...
        config: {
            ...
            events?: {
                ...
                onStateChange?: (event: { data: PlayerStateValue; target: PlayerInstance }) => void;
            };
        }
    ) => PlayerInstance;
};
...
const ADVENTURE_TUTORIAL_SLIDES: TutorialSlide[] = [
    {
        title: '🎼 스타트 규칙',
        lines: ['Play를 누르면 음악이 시작되고 재생 잠금이 켜집니다.', '도전 중에는 일시정지, 시크, 스테이지 스킵을 할 수 없어요.'],
        showArrows: false,
    },
    {
        title: '🦘 점프 조작',
        lines: ['코스 화면을 클릭하거나 터치하면 점프합니다.', '장애물이 리코 앞에 오기 전에 타이밍 맞춰 뛰어넘으세요.'],
        showArrows: false,
    },
    {
        title: '💾 세이브 포인트',
        lines: ['각 Phase를 끝까지 살아남으면 다음 Phase 체크포인트가 열립니다.', '게임 오버 후 열린 체크포인트에서 재도전할 수 있습니다.'],
        showArrows: false,
    },
];

const PHASES: Phase[] = [
    { id: 1, title: 'Phase 1', start: 0, end: 39, theme: '오프닝', sky: 'from-[#f9d8c2] via-[#f4f0d0] to-[#d4f0e2]', accent: '#9c6644', description: '용사 리코의 옛날이야기가 천천히 시작됩니다.' },
    { id: 2, title: 'Phase 2', start: 39, end: 121, theme: '출정', sky: 'from-[#f6c177] via-[#f9e2af] to-[#b8e0d2]', accent: '#b85c38', description: '긴 여정을 향해 첫 발을 내딛는 구간입니다.' },
    { id: 3, title: 'Phase 3', start: 121, end: 214, theme: '숲', sky: 'from-[#87b37a] via-[#d9ed92] to-[#f1f7d5]', accent: '#386641', description: '햇살이 스며드는 숲과 들판을 달립니다.' },
    { id: 4, title: 'Phase 4', start: 214, end: 282, theme: '던전', sky: 'from-[#283046] via-[#3d405b] to-[#111827]', accent: '#e07a5f', description: '마왕의 성 내부로 들어가며 분위기가 급격히 어두워집니다.' },
    { id: 5, title: 'Phase 5', start: 282, end: 310, theme: '결투', sky: 'from-[#4a1010] via-[#9d0208] to-[#ffba08]', accent: '#ffd166', description: '짧고 거대한 결전. 모든 에너지가 한곳에 몰립니다.' },
    { id: 6, title: 'Phase 6', start: 310, end: 388, theme: '승리', sky: 'from-[#6ec6ff] via-[#bde0fe] to-[#d8f3dc]', accent: '#2a9d8f', description: '전투가 끝나고 평화로운 풍경이 다시 펼쳐집니다.' },
    { id: 7, title: 'Phase 7', start: 388, end: 433, theme: '엔딩', sky: 'from-[#1d3557] via-[#457b9d] to-[#f1faee]', accent: '#f1fa8c', description: '옛날이야기의 마지막 페이지가 닫힙니다.' },
];

const getCurrentPhase = (time: number): Phase =>
    PHASES.find((phase) => time >= phase.start && time < phase.end) ?? PHASES[PHASES.length - 1];
...
const getVisibleObstacles = (time: number): Obstacle[] => {
    ...
    PHASES.forEach((phase) => {
        ...
        for (let marker = phase.start + offset; marker < phase.end; marker += step) {
            ...
            const emoji = phase.id <= 2
                ? (type === 'flare' ? '✨' : ['📜', '🪵', '🥁'][variant % 3])
                : phase.id === 3
                    ? (type === 'flare' ? '🦋' : ['🌲', '🍃', '🌿'][variant % 3])
                    : phase.id === 4
                        ? (type === 'flare' ? '🔥' : ['🪨', '🕸️', '🦴'][variant % 3])
                        : phase.id === 5
                            ? (type === 'flare' ? '💥' : ['⚔️', '👑', '🛡️'][variant % 3])
                            : (type === 'flare' ? '⭐' : ['🌼', '🎖️', '🕊️'][variant % 3]);
...
export default function AdventureGame() {
    ...
    const [deathMessage, setDeathMessage] = useState('장애물에 부딪혀 여정이 중단되었습니다.');

    const phase = useMemo(() => getCurrentPhase(timelineTime), [timelineTime]);
    const visibleObstacles = useMemo(() => getVisibleObstacles(timelineTime), [timelineTime]);
        ...
        playerRef.current = new window.YT.Player('rico-adventure-player', {
            ...
            events: {
                ...
                onStateChange: (event) => {
                    ...
                    if (event.data === 2 && runStateRef.current === 'running' && !intentionalPauseRef.current) {
    ...
    useEffect(() => {
        const tick = (ts: number) => {
            ...
            if (runState === 'running') {
                ...
                jumpYRef.current = Math.max(0, jumpYRef.current + jumpVelocityRef.current * delta);
    ...
    useEffect(() => {
        ...
        PHASES.forEach((item) => {
            if (timelineTime >= item.end) {
                const nextPhase = PHASES.find((candidate) => candidate.id === item.id + 1);
    ...
    useEffect(() => {
        ...
        for (const obstacle of visibleObstacles) {
            ...
            if (overlapsPlayer) {
                const hitGroundObstacle = obstacle.type === 'ground' && jumpYRef.current < GROUND_SAFE_HEIGHT;
                const hitFlareObstacle = obstacle.type === 'flare' && jumpYRef.current > FLARE_COLLISION_HEIGHT;
            ...
            if (x < PLAYER_X - 22) {
                ...
                if (obstacle.type === 'ground' && jumpYRef.current >= GROUND_SAFE_HEIGHT) {
    ...
    useEffect(() => {
        ...
        const award = async (code: string, title: string, description: string, icon: string) => {
        ...
        void award('LEGEND-HERO', '레전드 용사', '마왕을 물리치고 긴 여정을 끝마쳤다.', '⚔️');

        if (attemptStartPhaseIdRef.current === 1) {
            void award('R-GEND-HERO', 'R전드 용사', '1단계부터 끝까지 한 번에 여정을 완주했다.', '👑');
        }
    ...
    const handleJump = () => {
        if (runState !== 'running' || jumpYRef.current > 4) {
            return;
        }
    ...
    return (
        <GameContainer
            ...
            headerRight={
                <>
                    <div className="flex min-w-[92px] flex-col items-center rounded-2xl px-4 py-2" style={{ background: '#102542', color: '#FFFFF8' }}>
                        <span className="text-[10px] font-bold uppercase tracking-tighter opacity-70">Score</span>
                        <span className="text-xl font-black leading-tight">{currentScore}</span>
                    </div>
                    <div className="flex min-w-[92px] flex-col items-center rounded-2xl px-4 py-2" style={{ background: '#2a9d8f', color: '#fefae0' }}>
                        <span className="text-[10px] font-bold uppercase tracking-tighter opacity-80">Best</span>
                        <span className="text-xl font-black leading-tight">{bestScore}</span>
                    </div>
                </>
            }
        >
                ...
                <section className="grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
                    ...
                    >
                        <div
                            ...
                            style={{
                                backgroundImage: 'radial-gradient(circle at 20% 20%, rgba(255,255,255,0.55), transparent 28%), radial-gradient(circle at 80% 10%, rgba(255,255,255,0.35), transparent 22%), linear-gradient(180deg, rgba(255,255,255,0.08), transparent 35%)',
                            }}
                        />

                        <div className="relative z-10 mb-4 flex flex-wrap items-center justify-between gap-3">
                            <div>
                                <p className="text-sm font-black uppercase tracking-[0.28em]" style={{ color: phase.accent }}>
                                    {phase.title} · {phase.theme}
                                </p>
                                <h2 className="text-2xl font-black text-[#102542] sm:text-3xl">{phase.description}</h2>
                            </div>
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{
                                    backgroundImage: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.8) 30%, transparent 60%)',
                            ...
                            <div className="absolute left-0 right-0 top-4 flex justify-center">
                                <div className="rounded-full bg-[#102542]/70 px-4 py-2 text-sm font-bold text-white">
                                    Course {Math.round(progress * COURSE_LENGTH)}m / {COURSE_LENGTH}m
                                </div>
                            </div>
                                ...
                                return (
                                    <motion.div
                                        ...
                                        style={{
                                            ...
                                            filter: isPast ? 'grayscale(0.25)' : 'drop-shadow(0 8px 16px rgba(0,0,0,0.18))',
                                        }}
                                    >
                                        {obstacle.emoji}
                                    </motion.div>
                    ...
                    <div className="flex flex-col gap-5">
                        <section className="rounded-[2rem] border-4 border-[#102542]/10 bg-white/80 p-5 shadow-[0_18px_40px_rgba(17,24,39,0.08)]">
                            ...
                            <div className="overflow-hidden rounded-[1.4rem] border border-[#102542]/10 bg-[#102542]">
                                <div id="rico-adventure-player" className="aspect-video w-full pointer-events-none" />
                            </div>
                            ...
                            <div className="mt-4 grid gap-3">
                                {runState === 'ready' && (
                                    <button
                                        ...
                                        className="w-full rounded-[1.2rem] bg-[#102542] px-5 py-4 text-base font-black text-white transition-transform hover:scale-[1.01] disabled:cursor-wait disabled:opacity-60"
                                    >
                        ...
                        <section className="rounded-[2rem] border-4 border-[#102542]/10 bg-white/80 p-5 shadow-[0_18px_40px_rgba(17,24,39,0.08)]">
                            <div className="mb-3 flex items-center justify-between gap-3">
                                <h3 className="text-lg font-black text-[#102542]">Phase Save Points</h3>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-3">
                                                <div>
                                                    <p className="text-xs font-black uppercase tracking-[0.22em] opacity-70">{item.title}</p>
            ...
            {runState === 'gameover' && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <h2 className="mt-3 text-2xl font-black text-[#102542]">Game Over</h2>
                        <p className="mt-2 text-sm font-bold text-[#5c677d]">{deathMessage}</p>
                        <div className="mt-4 grid grid-cols-2 gap-3 text-sm font-black">
                            <div className="rounded-2xl bg-[#102542] px-4 py-3 text-white">
                                <div className="text-[10px] uppercase tracking-[0.22em] opacity-70">Score</div>
                                <div className="text-xl">{currentScore}</div>
                            </div>
                            <div className="rounded-2xl bg-[#2a9d8f] px-4 py-3 text-white">
                                <div className="text-[10px] uppercase tracking-[0.22em] opacity-80">Best</div>
                                <div className="text-xl">{bestScore}</div>
                            </div>
                        </div>
                        <p className="mt-4 text-xs font-bold uppercase tracking-[0.22em] text-[#102542]">
                            Retry from {PHASES.find((item) => item.id === selectedRetryPhaseId)?.title}
                        </p>
                        <button
                            type="button"
                            onClick={handleRetry}
